      ...
      <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between">
        <div>
          <div className="flex justify-between items-center mb-2">
            <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider block">Total Hama Terperangkap</span>
            <span className="text-[10px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-100 px-2 py-0.5 rounded-full flex items-center gap-0.5">
              <i className="bi bi-arrow-up-short"></i> {trendData.summary.pestChange}
            </span>
          </div>
          <div className="flex items-baseline space-x-2 my-2">
            <span className="text-3xl font-extrabold text-gray-800">{trendData.summary.totalPests.toLocaleString('id-ID')}</span>
            <span className="text-xs text-gray-400 font-medium">ekor</span>
          </div>
        </div>
        <div className="mt-3">
          <div className="w-full bg-gray-100 h-1.5 rounded-full overflow-hidden mb-1">
            <div className="bg-emerald-600 h-full" style={{ width: `${Math.min((trendData.summary.totalPests / 1500) * 100, 100)}%` }}></div>
          </div>
          <span className="text-[10px] text-gray-400 font-semibold block">Target eliminasi: 1.500/bulan</span>
        </div>
      </div>
