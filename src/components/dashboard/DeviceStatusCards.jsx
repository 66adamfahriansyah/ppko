function DeviceStatusCards({ data, isLoaded }) {
  // Pengecekan apakah data dari MQTT telah diterima
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {/* Rain Sensor Card */}
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col justify-between min-h-[200px]">
        <div>
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-medium bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-full border border-emerald-100">
                <i className="bi bi-cpu-fill mr-1"></i>Fuzzy 4 Node
              </span>
        ...
        <div>
          <div className="flex justify-between items-center text-xs mb-1.5">
            <span className="text-gray-400 font-medium">Tingkat Curah Hujan (Validasi Fuzzy)</span>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col justify-between min-h-[200px]">
        <div>
          <div className="flex justify-between items-center mb-3">
            <h5 className="text-sm font-semibold text-gray-600 flex items-center">
              <i className="bi bi-bug-fill text-emerald-600 mr-2 text-base"></i>Status Relay Light Trap
            </h5>
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-medium bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-full border border-emerald-100">
                <i className="bi bi-diagram-2-fill mr-1"></i>Fuzzy 4 Node
              </span>
              <span className={`w-2.5 h-2.5 rounded-full ${isLoaded && data.lightTrap.active ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-red-500'}`}></span>
            </div>
          </div>
